...
State2D::State2D(const Pose2D &p, Gear g, double vel, double wheel_angle,
...
bool State2D::operator==(const State2D &s) {
    return (0.0001 > std::fabs(s.position.Distance2(position)) &&
            0.001 > std::fabs(s.orientation - orientation));
}
