...
Pose2D::Pose2D(const Vector2D<double> &p, double o)
...
bool Pose2D::operator==(const Pose2D &pose) {
    return (0.0001 > position.Distance2(pose.position) &&
            0.1 > std::fabs(pose.orientation - orientation));
}

// differet operator overloading
bool Pose2D::operator!=(const Pose2D &pose) {
    return (0.0001 < std::fabs(position.Distance2(position)) ||
            0.1 < std::fabs(pose.orientation - orientation));
}
