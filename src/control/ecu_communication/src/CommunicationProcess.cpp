...
//    this->params_.fake_issue = false;
        ...
        if (this->yaml_params_.lower_layer_receive || this->yaml_params_.lower_layer_send) {
            ...
            if (this->yaml_params_.lower_layer_send) {
                this->udp_send_proportion_.inputProportion(1, 1, 1);
        ...
        if (this->yaml_params_.upper_layer_send || this->yaml_params_.upper_layer_receive) {
            ...
            if (this->yaml_params_.upper_layer_send) {
                this->ros_publish_timer_ = this->nh_.createTimer(ros::Duration(PUBLISH_PERIOD),
                                                                 boost::bind(&AutonomousControl::dataProcess,
                                                                             &this->autonomousControl_));
            }
        ...
        if (this->yaml_params_.remote_send || this->yaml_params_.remote_receive) {
            this->remoteControl_.init(&this->data_download_,
                                      &this->data_upload_,
                                      &this->data_download_mutex_,
                                      &this->data_upload_mutex_,
                                      &this->sLog_,
                                      &this->control_mode_,
                                      &this->control_mode_mutex_,
                                      &this->gps_,
                                      this->yaml_params_.files,
                                      &this->halt_by_remote);
            if (this->yaml_params_.remote_send) {
                this->remote_send_timer_ = this->nh_.createTimer(ros::Duration(REMOTE_SEND_PERIOD),
                                                                 boost::bind(&RemoteControl::dataSend,
                                                                             &this->remoteControl_));
            }
...
//    this->serialPortCommunication.colse();
    ...
    void CommunicationProcess::paramsInit() {
        ...
        tmp_result &=
            this->private_nh_.getParam("send_default_when_no_msg", this->yaml_params_.send_default_when_no_msg);
...
//        std::cout << std::endl;
            ...
            if (this->yaml_params_.publish_rawdata) {
                ...
                this->data_upload_.recv_rawdata.data.assign(this->udp_.buffer,
                                                            this->udp_.buffer + this->udp_.get_recv_len());
    ...
    void CommunicationProcess::udpSend() {
        ...
        if (!this->udp_.sendToRemote(this->data_download_.data_to_send, sizeof(this->data_download_.data_to_send))) {
            ...
            this->sLog_.logUint8Array((char *) this->data_download_.data_to_send,
                                      sizeof(this->data_download_.data_to_send),
                                      google::ERROR);
            return;
        }
        if (this->yaml_params_.log_rawdata) {
        ...
        if (this->yaml_params_.publish_rawdata) {
            ...
            this->data_download_.send_rawdata.data.assign(this->data_download_.data_to_send,
                                                          this->data_download_.data_to_send
                                                              + sizeof(this->data_download_.data_to_send));
...
//    pack_recv_till_now_check = this->pack_recv_times_.checkTimestampsTillNow(-1, -1);
        ...
        return (udp_recv_duration_check && udp_recv_till_now_check && pack_recv_duration_check
            && pack_recv_till_now_check);
//    return true;
    }
