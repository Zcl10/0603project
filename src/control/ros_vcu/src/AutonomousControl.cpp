...
bool AutonomousControl::rosmsgUpdateCheck() {
    //// todo modify check
    p_data_download_->speed_cmd_receive = this->msg_update_times.checkSingleTimestampTillNow(this->speed_sub_handle_, -1, 200);
    p_data_download_->steer_cmd_receive = this->msg_update_times.checkSingleTimestampTillNow(this->steer_sub_handle_, -1, 200);
    p_data_download_->mode_cmd_receive = this->msg_update_times.checkSingleTimestampTillNow(this->mode_sub_handle_, -1, 200);
...
void AutonomousControl::speedCb(const control_msgs::ControlSpeed &msg) {
    ...
    this->p_data_download_->SendECUData.expect_speed = (msg.speed/0.06875*3.6);
    this->p_data_download_mutex_->unlock();
    this->msg_update_times.pushTimestamp(this->speed_sub_handle_);
}
