...
DataUpload::DataUpload() {
    ...
    memset(this->BrakeFromVcu,0, sizeof(this->BrakeFromVcu));
    memset(this->TorqueFromVcu,0, sizeof(this->TorqueFromVcu));
...
void DataUpload::dataDistribution() {
    ...
    switch (this->pack_handle.getID()) {
        ...
        case 5: {
            memcpy(this->BrakeFromVcu, this->recv_raw_data, sizeof(this->recv_raw_data));
            break;
        }
        case 6: {
            memcpy(this->TorqueFromVcu, this->recv_raw_data, sizeof(this->recv_raw_data));
            break;
        }
        default: {
            --this->recv_counter;
...
void DataUpload::dataToMsg() {
    //0x186
    this->ecuReport.vehicle_work_mode.work_mode = (this->VehicleWorkMode[7]&0x3C);
    ...
    this->ecuReport.wheel.front_left = (((this->WheelSpeed[5]>>1)&0x7F) + (this->WheelSpeed[6]&0x1F)*128)*0.06875/3.6;

    this->ecuReport.wheel.statusfr = ((this->WheelSpeed[6]>>5)&0x01);
    this->ecuReport.wheel.front_right  =
            ((this->WheelSpeed[6]>>6) +(this->WheelSpeed[7])*4+ (this->WheelSpeed[8]&0x03)*1024)*0.06875/3.6;

    this->ecuReport.wheel.statusrl = ((this->WheelSpeed[8]>>2)&0x01);
    this->ecuReport.wheel.rear_left = ((this->WheelSpeed[8]>>3) + (this->WheelSpeed[9]&0x7F)*32)*0.06875/3.6;

    this->ecuReport.wheel.statusrr = ((this->WheelSpeed[9]>>7)&0x01);
    this->ecuReport.wheel.rear_right =((this->WheelSpeed[10]) +(this->WheelSpeed[11]&0x0F)*256)*0.06875/3.6;
    //0x185
    ///车速
    ...
    this->ecuReport.brake.brake_pedal = ((this->gear_steer_throttle_brakedeepth[10]>>4)
     ...
     if(ecuReport.brake.brake_pedal>0)
     {
         ecuReport.manual.is_human_brake = 1;

     }
     //510 3A4 brake & torque from vcu
     this->ecuReport.vcu_torque_brake.Speed.brake = this->BrakeFromVcu[5]*0.1;
    this->ecuReport.vcu_torque_brake.Speed.torque = ((this->TorqueFromVcu[6]+this->TorqueFromVcu[7]*256)-12000);
    //0x505
    ///YuanFeng Esc制动反馈
